...
def flip_image(gl_img):
    """
    Flip an image Pillow.
    """
    img = Image.open(StringIO.StringIO(gl_img._image_data))
    img = img.transpose(Image.FLIP_LEFT_RIGHT)
    return from_pil_image(img)

def flop_image(gl_img):
    """
    Flop an image Pillow.
    """
    img = Image.open(StringIO.StringIO(gl_img._image_data))
...
def random_rotate(x):
    ...
    a = x['id'] % 4
    if a == 0:
        return rotate_image(x['image'], 45)
    elif a == 1:
        return rotate_image(x['image'], 90)
    elif a == 2:
        return rotate_image(x['image'], 135)
    elif a == 3:
        return rotate_image(x['image'], 180)
    elif a == 4:
        return rotate_image(x['image'], 225)
    elif a == 5:
        return rotate_image(x['image'], 270)
    elif a == 6:
        return rotate_image(x['image'], 315)
    # elif a == 3:
    #     return flip_image(x['image'])
    # elif a == 4:
    #     return flop_image(x['image'])
    elif a == 7:
        return x['image']
...
if __name__ == "__main__":
    ...
    train['image'] = gl.image_analysis.resize(train['image'], 96, 96, 3)
    ...
    train, valid = gl.recommender.util.random_split_by_user(train, 
                   user_id='class', item_id='image', item_test_proportion=0.25)
    valid = valid.add_row_number()

    # Perform the data augmentation by making 4 copies of the data.
    print "Data Augmentation..."
    trainbk=train
    for i in range(7):
        train = train.append(trainbk)
    ...
    model = gl.neuralnet_classifier.create(train, 'class', 
                                           ...
                                           random_mirror=True, 
                                           random_crop=True,
                                           model_checkpoint_path='sriv2',
                                           model_checkpoint_interval=5
                                           )

    #Save the model
    model.save('model_stdrot_ep50_v2')
    ...
    test['image'] = gl.image_analysis.resize(test['image'], 96, 96, 3)
