...
#         result.append(fast_warp(img, tform_ds + tform_augment, output_shape=target_size, mode='reflect').astype('float32'))
          result.append(warp(img,tform_ds+tform_augment, output_shape=target_size,mode='nearest'))
...
def load_and_process_images(folder):
    ...
    for fileNameDir in os.walk(folder):   
        for fileName in fileNameDir[2]:
            ...
            # tform_ds=[tform_ds_cropped33]

            tform_ds = [
                build_ds_transform(2.0, orig_size=(row,col), target_size=target_size), 
                build_ds_transform(2.0, orig_size=(row,col), target_size=target_size) + build_augmentation_transform(orig_size=(row,col), rotation=45)]
...
def load_and_process_test_images(fl):
    i=0
    img = imread(fl)
    row,col=img.shape
    global aug_transform
    # tform_ds_8x = build_ds_transform(8.0, orig_size=(row,col), target_size=target_size, do_shift=True, subpixel_shift=False)
    # tform_ds_cropped33 = build_ds_transform(3.0, orig_size=(row,col), target_size=target_size, do_shift=True, subpixel_shift=False)
    # tform_ds=[tform_ds_cropped33]
    ...
    aug_transform=build_augmentation_transform(orig_size=(row,col),rotation=angle, zoom=zoom, shear=shear)
    # print angle,zoom,shear
    # spimgf = imgf.split('/')
    # # image_path = '/'.join(spimgf[:-1])
    # image_path=fo+spimgf[:-1][-1]
    # # print image_path
...
def master_joblib_loop(directory_names="",file_names=""):
    if loop=='train':
        Parallel(n_jobs=-1)(delayed(load_and_process_images)(folder) for i,folder in enumerate(directory_names))
    else:
        Parallel(n_jobs=-1)(delayed(load_and_process_test_images)(fl) for fl in file_names)
...
def process_train(fi,fo,target_size):
    ...
    master_joblib_loop(directory_names=directory_names)


def process_test(fi,fo,target_size):

    file_names = list(glob.glob(os.path.join(fi, "*.jpg")))

    # print directory_names
    master_joblib_loop(file_names=file_names)
...
if __name__ == "__main__":
    ...
    shear=float(sys.argv[7])
    if loop=='train':
        process_train(fi,fo,target_size)
    else:
        process_test(fi,fo,target_size)
